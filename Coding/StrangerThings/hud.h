...
#include <QVector> // Qt container used to store multiple enemies inside a list
...
private: // All functions will need to be pointers because they are owned somehwere else and HUD class only uses them doesnt own them
    ...
    QGraphicsTextItem* levelInfo; // This shows the current level number on screen
...
public:
    ...
    // Updated signature: also receives level number and all enemies
    void update(Player* p, Enemy* primaryEnemy, bool isPlayerTurn,
                int levelNum, const QVector<Enemy*>& allEnemies); // This updates all HUD information based on current game status including level and all enemies
