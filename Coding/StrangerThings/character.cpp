...
Character::Character(QString name, int hp, int atk, int def, Position pos)
    : name(name), health(hp), maxHealth(hp), attackPower(atk), defense(def), position(pos), alive(true) {}
...
QString  Character::getName()     const { return name; } // Returns the characters name
